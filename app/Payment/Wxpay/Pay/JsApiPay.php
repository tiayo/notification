...
class JsApiPay
{
	...
	public function GetOpenid($order_id)
	{
		...
		if (!isset($_GET['code'])){
			//触发微信返回code码
			$baseUrl = urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].$_SERVER['QUERY_STRING'].'/'.$order_id);
	...
	{
		$urlObj["appid"] = config('wxpay.APPID');
	...
	{
		$urlObj["appid"] = config('wxpay.APPID');
		$urlObj["secret"] = config('wxpay.APPSECRET');
		$urlObj["code"] = $code;
